...
namespace Spellbound.Modifiers.Samples {
    /// <summary>
    /// Sample enemy modifier: adds flat <c>cold_resistance</c> to a <see cref="ResistanceBehaviour"/> — the same
    /// shape as <see cref="FireResistanceModifier"/>, pointed at a different element.
    /// </summary>
    [Serializable, PackerId("sample_increased_cold_resistance")]
    public sealed class ColdResistanceModifier : SbModifier {
        ...
        public override void Apply(ICanBeModified target) {
            if (TryGetBehaviour<ResistanceBehaviour>(target, out var resistance))
                resistance.AddFlat("sample_cold_resistance", amount, UniqueId);
        }
        ...
        public override ISmartPacker CreateNewInstance() => new ColdResistanceModifier();
    }
}
