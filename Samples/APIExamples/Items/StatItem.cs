...
namespace Spellbound.Modifiers.Samples {
    ...
    public sealed class StatItem : ModifiableItem {
        private readonly (StatId stat, ModifierType type, float value)[] _entries;

        public StatItem(params (StatId stat, ModifierType type, float value)[] entries) => _entries = entries;

        protected override void OnEquip(Modifiable target) {
            foreach (var (stat, type, value) in _entries)
                target.Stats.AddModifier(stat, type, value, SourceId);
        }
    }
}
